      ...
      <div class="row">
      	...
      	<?php echo $department_tab;?>
      		<div class="col-lg-12" id="loginform">
      		 	<h2>แก้ไข สาขาวิชา/งาน</h2>
      		 	<form role="form" class="form-inline" action="?d=manage&c=department&m=search" method="post" autocomplete="off">
      			...
      			<form role="form" action="?d=manage&c=department&m=edit" method="post" autocomplete="off">
...
<!-- Custom Javascript -->
	...
	$(function(){
		...
		$("#allow-all").click(function(){
			$(".allow_department0:not(:checked)").each(function(){
				$(this).prop("checked",true);
			});
			$(".allow_department1:not(:checked)").each(function(){
				$(this).prop("checked",true);
			});
		});
		/*#################################################
		...
		$("#disallow-all").click(function(){
			$(".allow_department1:checked").each(function(){
				$(this).prop("checked",false);
			});
			$(".allow_department0:checked").each(function(){
				$(this).prop("checked",false);
			});
		});
		/*#################################################
		...
		###################################################*/
		$("#del_all_department").click(function(e){
			if(this.checked)
			{
				$(".del_department").prop("checked",true);
			}
			else $(".del_department").prop("checked",false);		
		});
		...
		$("#clearSearch").click(function(){
			$.post("<?php echo base_url()?>?d=manage&c=department&m=search",{clear:"clear"},function(data,status){
				window.location="<?php echo base_url();?>?d=manage&c=department&m=edit";
			});
		}); 
		/*#################################################
		...
		###################################################*/
		<?php 
		if($this->session->flashdata("edit_department_message"))
		{?>
			bootbox.alert("<?php echo $this->session->flashdata("edit_department_message");?>"); 
		<?php
		}?>
	...
	function load_department(tid)
	{
		//show data in input
		$.ajax({
			url:"?d=manage&c=department&m=load_department",
			data:"tid="+tid,
			type:"POST",
			dataType:"json",
	...
	function show_del_list()
	{
		...
		var checked_num = $(".del_department:checked").length;
		var text = "ลบทั้งหมด "+checked_num+" รายการ?<br/>";
		text+="<ul>";
		$(".del_department").each(function(){
			if(this.checked)
			text+="<li>"+$(this).val()+" "+$("#department"+$(this).val()).text()+"</li>";
		});
		text+="</ul>";
		bootbox.confirm(text,function(result){
			if(result==true && checked_num>0)$("#form_del_department").submit();
		});
	}
	...
	function select_orderby()
	{
		...
		select_field+='<option value="department_id">รหัส สาขาวิชา/งาน</option>';
		select_field+='<option value="department_name">สาขาวิชา/งาน</option>';
		select_field+='</select>';
			...
			buttons: {
				success: {
					...
					callback: function() {
						...
						$.post("<?php echo base_url()?>?d=manage&c=department&m=set_orderby",{field:$("#orderby").val(),type:$("#ordertype").val()},function(data,status){
							window.location="<?php echo base_url();?>?d=manage&c=department&m=edit";
						});
					}
				},
		...
		$("#orderby").val("<?php echo $this->session->userdata("orderby_department")["field"];?>");
		$("#ordertype").val("<?php echo $this->session->userdata("orderby_department")["type"];?>");
	}
	function show_allow_list()
	{
		...
		var allow_num = $(".allow_department0:checked").length;
		var disallow_num = $(".allow_department1:not(:checked)").length;
		...
		$(".allow_department1:not(:checked)").each(function(){
				...
				text2+="<li>"+$(this).val()+" "+$("#department"+$(this).val()).text()+"</li>";
		...
		bootbox.confirm(text+text2,function(result){
			...
			{
				$.post("<?php echo base_url()?>?d=manage&c=department&m=allow",{allow_list:allow_list,disallow_list:disallow_list},function(data,status){
					window.location="<?php echo base_url();?>?d=manage&c=department&m=edit";
				});
			}
		});
