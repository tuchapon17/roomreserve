      ...
      <div class="row">
      	...
      	<?php echo $faculty_tab;?>
      		<div class="col-lg-12" id="loginform">
      		 	<h2>แก้ไข คณะ/กอง</h2>
      		 	<form role="form" class="form-inline" action="?d=manage&c=faculty&m=search" method="post" autocomplete="off">
      			...
      			<form role="form" action="?d=manage&c=faculty&m=edit" method="post" autocomplete="off">
...
<!-- Custom Javascript -->
	...
	$(function(){
		...
		$("#allow-all").click(function(){
			$(".allow_faculty0:not(:checked)").each(function(){
				$(this).prop("checked",true);
			});
			$(".allow_faculty1:not(:checked)").each(function(){
				$(this).prop("checked",true);
			});
		});
		/*#################################################
		...
		$("#disallow-all").click(function(){
			$(".allow_faculty1:checked").each(function(){
				$(this).prop("checked",false);
			});
			$(".allow_faculty0:checked").each(function(){
				$(this).prop("checked",false);
			});
		});
		/*#################################################
		...
		###################################################*/
		$("#del_all_faculty").click(function(e){
			if(this.checked)
			{
				$(".del_faculty").prop("checked",true);
			}
			else $(".del_faculty").prop("checked",false);		
		});
		...
		$("#clearSearch").click(function(){
			$.post("<?php echo base_url()?>?d=manage&c=faculty&m=search",{clear:"clear"},function(data,status){
				window.location="<?php echo base_url();?>?d=manage&c=faculty&m=edit";
			});
		}); 
		/*#################################################
		...
		###################################################*/
		<?php 
		if($this->session->flashdata("edit_faculty_message"))
		{?>
			bootbox.alert("<?php echo $this->session->flashdata("edit_faculty_message");?>"); 
		<?php
		}?>
	...
	function load_faculty(tid)
	{
		//show data in input
		$.ajax({
			url:"?d=manage&c=faculty&m=load_faculty",
			data:"tid="+tid,
			type:"POST",
			dataType:"json",
	...
	function show_del_list()
	{
		...
		var checked_num = $(".del_faculty:checked").length;
		var text = "ลบทั้งหมด "+checked_num+" รายการ?<br/>";
		text+="<ul>";
		$(".del_faculty").each(function(){
			if(this.checked)
			text+="<li>"+$(this).val()+" "+$("#faculty"+$(this).val()).text()+"</li>";
		});
		text+="</ul>";
		bootbox.confirm(text,function(result){
			if(result==true && checked_num>0)$("#form_del_faculty").submit();
		});
	}
	...
	function select_orderby()
	{
		...
		select_field+='<option value="faculty_id">รหัส คณะ/กอง</option>';
			...
			buttons: {
				success: {
					...
					callback: function() {
						...
						$.post("<?php echo base_url()?>?d=manage&c=faculty&m=set_orderby",{field:$("#orderby").val(),type:$("#ordertype").val()},function(data,status){
							window.location="<?php echo base_url();?>?d=manage&c=faculty&m=edit";
						});
					}
				},
		...
		$("#orderby").val("<?php echo $this->session->userdata("orderby_faculty")["field"];?>");
		$("#ordertype").val("<?php echo $this->session->userdata("orderby_faculty")["type"];?>");
	}
	function show_allow_list()
	{
		...
		var allow_num = $(".allow_faculty0:checked").length;
		var disallow_num = $(".allow_faculty1:not(:checked)").length;
		...
		$(".allow_faculty1:not(:checked)").each(function(){
				...
				text2+="<li>"+$(this).val()+" "+$("#faculty"+$(this).val()).text()+"</li>";
		...
		bootbox.confirm(text+text2,function(result){
			...
			{
				$.post("<?php echo base_url()?>?d=manage&c=faculty&m=allow",{allow_list:allow_list,disallow_list:disallow_list},function(data,status){
					window.location="<?php echo base_url();?>?d=manage&c=faculty&m=edit";
				});
			}
		});
