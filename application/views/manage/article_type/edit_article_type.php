      ...
      <div class="row">
      	...
      	<?php echo $article_type_tab;?>
      		<div class="col-lg-12" id="loginform">
      		 	<h2>แก้ไขประเภทครุภัณฑ์/อุปกรณ์</h2>
      		 	<form role="form" class="form-inline" action="?d=manage&c=article_type&m=search" method="post">
      			...
      			<form role="form" action="?d=manage&c=article_type&m=edit" method="post">
...
<!-- Custom Javascript -->
	...
	$(function(){
		...
		###################################################*/
		$("#del_all_article_type").click(function(e){
			if(this.checked)
			{
				$(".del_article_type").prop("checked",true);
			}
			else $(".del_article_type").prop("checked",false);		
		});
		...
		$("#clearSearch").click(function(){
			$.post("<?php echo base_url()?>?d=manage&c=article_type&m=search",{clear:"clear"},function(data,status){
				window.location="<?php echo base_url();?>?d=manage&c=article_type&m=edit";
			});
		}); 
		/*#################################################
		...
		###################################################*/
		<?php 
		if($this->session->flashdata("edit_article_type_message"))
		{?>
			bootbox.alert("<?php echo $this->session->flashdata("edit_article_type_message");?>"); 
		<?php
		}?>
	...
	function load_article_type(tid)
	{
		//show data in input
		$.ajax({
			url:"?d=manage&c=article_type&m=load_article_type",
			data:"tid="+tid,
			type:"POST",
			dataType:"json",
	...
	function show_del_list()
	{
		...
		var checked_num = $(".del_article_type:checked").length;
		var text = "ลบทั้งหมด "+checked_num+" รายการ?<br/>";
		text+="<ul>";
		$(".del_article_type").each(function(){
			if(this.checked)
			text+="<li>"+$(this).val()+" "+$("#article_type"+$(this).val()).text()+"</li>";
		});
		text+="</ul>";
		bootbox.confirm(text,function(result){
			if(result==true && checked_num>0)$("#form_del_article_type").submit();
		});
	}
	function set_orderby(field)
	{
		<?php 
		if($this->session->userdata("orderby_article_type")["type"]=="ASC") $type="DESC";
		else if($this->session->userdata("orderby_article_type")["type"]=="DESC") $type="ASC"
		?>
		$.post("<?php echo base_url()?>?d=manage&c=article_type&m=set_orderby",{field:field,type:"<?php echo $type;?>"},function(data,status){
			window.location="<?php echo base_url();?>?d=manage&c=article_type&m=edit";
		});
	}
	function select_orderby()
	{
		...
		select_field+='<option value="article_type_id">รหัสประเภทครุภัณฑ์/อุปกรณ์</option>';
			...
			buttons: {
				success: {
					...
					callback: function() {
						...
						$.post("<?php echo base_url()?>?d=manage&c=article_type&m=set_orderby",{field:$("#orderby").val(),type:$("#ordertype").val()},function(data,status){
							window.location="<?php echo base_url();?>?d=manage&c=article_type&m=edit";
						});
					}
				},
		...
		$("#orderby").val("<?php echo $this->session->userdata("orderby_article_type")["field"];?>");
		$("#ordertype").val("<?php echo $this->session->userdata("orderby_article_type")["type"];?>");
	}
	//-->
